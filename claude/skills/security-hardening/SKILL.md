...
description: |
  ...
  注: `security-guidance` plugin がある環境では、コード編集時の自動検出はそちらが担当する。
  本スキルは「能動的ハードニング」「境界レビュー」「依存スキャン」「総点検レポート」など、
  自動レビューでは代替できない用途に使用する。plugin が無い環境では自動検出も本スキルで補う。
---
...
自動レビュー(per-edit / end-of-turn / commit)でカバーされない、以下のような場面で
使用してください:
...
## When NOT to Use(自動レビューに任せる場面)

`security-guidance` plugin がある環境では、以下は自動でカバーされるため明示呼び出し不要:

- コード編集中の脆弱性検出(per-edit パターンマッチ)
- ターン終了時の差分セキュリティレビュー
- `git commit` / `git push` 時のエージェント型レビュー

自動レビューの検出結果に対する追加調査や、より深い設計観点の強化が必要な場合に
本スキルを併用してください。

plugin が無い環境(Codex 等)ではこの自動レビューが働かない。上記も本スキルの対象に
含め、コード変更を伴う作業では PR 前に一度通す。
