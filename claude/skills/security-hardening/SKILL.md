...
description: |
  OWASP Top 10に準拠した能動的なセキュリティ強化を支援。設計段階の脅威モデリング、
  依存関係の脆弱性スキャン、構造化されたセキュリティレポート生成を実施。
  「セキュリティ強化して」「OWASP準拠を確認」「依存関係の脆弱性をチェック」
  「脅威モデリング」などの指示で起動。

  注: コード編集時の自動検出は `security-guidance` plugin が担当する。
  本スキルは「能動的ハードニング」「依存スキャン」「総点検レポート」など、
...
このスキルは、OWASP Top 10に基づいた**能動的な**セキュリティ強化を提供します。
`security-guidance` plugin の自動レビュー（per-edit / end-of-turn / commit）で
カバーされない、以下のような場面で使用してください:

- 新機能着手時の設計レビュー・脅威モデリング
- PR前の総点検と構造化レポート生成
- 依存関係の脆弱性スキャン（`npm audit`, `pip-audit` 等の実行）
- OWASP Top 10 コンプライアンス状況の明示的な確認
...
- OWASP Top 10準拠の確認（コンプライアンス報告含む）
